class ContactsController < ApplicationController
  before_action :set_contact, only: %i[ show edit update destroy ]

  # GET /contacts or /contacts.json
  def index
    @contacts = Contact.all
  end

  # GET /contacts/1 or /contacts/1.json
  def show
    @contact = Contact.includes(:company).find(params[:id]) # Inclui a empresa associada
    render json: @contact.as_json(
      only: [:id, :first_name, :last_name, :email, :phone],
      include: {
        company: { only: [:cnpj, :company_name] } # Inclui os campos da empresa
      }
    )
  end

  # GET /contacts/new
  def new
    @contact = Contact.new
  end

  # GET /contacts/1/edit
  def edit
  end

  # POST /contacts or /contacts.json
  def create
    @contact = Contact.new(contact_params)

    respond_to do |format|
      if @contact.save
        @contact.companies << Company.find(contact_params[:company_id])
        format.html { redirect_to @contact, notice: "Oppa! Contato criado com sucesso!." }
        format.json { render :show, status: :created, location: @contact }
      else
        format.html { render :new, status: :unprocessable_entity }
        format.json { render json: @contact.errors, status: :unprocessable_entity }
      end
    end
  end

  # PATCH/PUT /contacts/1 or /contacts/1.json
  def update
    respond_to do |format|
      if @contact.update(contact_params)
        format.html { redirect_to @contact, notice: "Oppa! Contato criado com sucesso!." }
        format.json { render :show, status: :ok, location: @contact }
      else
        format.html { render :edit, status: :unprocessable_entity }
        format.json { render json: @contact.errors, status: :unprocessable_entity }
      end
    end
  end

  # DELETE /contacts/1 or /contacts/1.json
  def destroy
    @contact.destroy!

    respond_to do |format|
      format.html { redirect_to contacts_path, status: :see_other, notice: "Contact was successfully destroyed." }
      format.json { head :no_content }
    end
  end

  # BUSCA DO CONTATO - OPPORTUNITY
  def search
    query = params[:query]
    if query.present?
      contacts = Contact.where("first_name ILIKE ? OR last_name ILIKE ?", "%#{query}%", "%#{query}%")
      render json: contacts
    else
      render json: []
    end
  end
 
  private
    # Use callbacks to share common setup or constraints between actions.
    def set_contact
      @contact = Contact.find(params[:id]) # Corrigido: use params[:id]
    end

    # Only allow a list of trusted parameters through.
    def contact_params
      params.require(:contact).permit(
        :first_name, :last_name, :phone, :email, :company_id, :role, :department
      ) 
    end
end